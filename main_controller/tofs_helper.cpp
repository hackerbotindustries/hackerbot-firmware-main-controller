...
/* --------------------------------  Compare Left Result  -------------------------------*/
bool compare_left_result(VL53L7CX_ResultsData *Result) {
  int8_t i, j, k;
...
bool check_left_sensor() {
  ...
  if (tofs_left_state == TOFS_STATE_READY) {
    ...
    if ((!statusLeft) && (NewDataReadyLeft != 0)) {
      ...
      return compare_left_result(&Results);
    }
  }
...
bool compare_right_result(VL53L7CX_ResultsData *Result) {
  int8_t i, j, k;
  uint8_t zones_per_line;
  uint8_t number_of_zones = VL53L7CX_RESOLUTION_4X4;
...
bool check_right_sensor() {
  ...
  if (tofs_right_state == TOFS_STATE_READY) {
    ...
    if ((!statusRight) && (NewDataReadyRight != 0)) {
      ...
      return compare_right_result(&Results);
    }
  }
